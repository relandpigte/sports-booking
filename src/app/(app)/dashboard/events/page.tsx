          ...
          <div className="divide-y divide-slate-100">
            {events.map((event) => (
              <article key={event.id} className="grid gap-4 px-5 py-5 sm:grid-cols-[1.5fr_1fr_0.8fr_auto] sm:items-center sm:px-6">
                ...
                <div className="flex flex-wrap items-start gap-2 sm:justify-end"><Link href={`/events/${event.publicId}`} className="rounded-lg px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50">View</Link><Link href={`/dashboard/events/${event.publicId}/edit`} className="rounded-lg bg-primary-soft px-3 py-2 text-xs font-bold text-primary hover:bg-primary/15">Manage</Link>{event.status === "CANCELLED" ? <DeleteCancelledEventButton eventId={event.id} title={event.title} /> : null}</div>
              </article>
            ))}
          </div>
        </div>
