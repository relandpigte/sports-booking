...
export default async function TrainerSchedulePage() {
  ...
  return <div className="space-y-6"><DashboardPageHeader eyebrow="Trainer tools" title="Your availability" description="Set recurring hours and date-specific exceptions. Existing sessions remain reserved." /><TrainerTabs /><TrainerScheduleForm rules={profile.weeklyRules.map(({ dayOfWeek, startHour, endHour }) => ({ dayOfWeek, startHour, endHour }))} exceptions={profile.exceptions.map(({ id, date, startHour, endHour, type, note }) => ({ id, date, startHour, endHour, type, note }))} /></div>;
}
