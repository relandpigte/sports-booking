  ...
  return (
    <PageShell maxWidth="max-w-7xl">
      ...
      <div className="py-6 sm:py-10">
        {event ? <Link href={`/events/${event.publicId}`} className="text-sm font-bold text-primary">← Event page</Link> : <Link href="/bunalq" className="text-sm font-bold text-primary">← Live BunalQ</Link>}
        <header className="my-5 rounded-3xl bg-navy p-5 text-white sm:p-7"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-accent">BunalQ · Live court rotation</p><h1 className="mt-2 text-3xl font-black">{snapshot.queue.title}</h1><p className="mt-2 text-sm text-white/65">{snapshot.queue.guestCreated ? "Guest-created Quick Queue" : `${snapshot.queue.hub.name}${event ? ` · ${formatManilaDateLong(event.date)} · ${formatSlotRange(event.startHour, event.endHour)}` : " · Quick Queue"}`}</p></div><span className={`rounded-full px-3 py-1 text-xs font-black ${snapshot.status === "ACTIVE" ? "bg-primary" : "bg-white/10"}`}>{snapshot.status}</span></div></header>
