...
export function CancelEventPanel({ eventId }: { eventId: string }) {
  ...
  return (
    <form action={action} className="rounded-2xl border border-red-200 bg-red-50 p-5">
      ...
      <p className="mt-1 text-sm leading-6 text-red-700">Court hours are released immediately. Successful payments are refunded by default.</p>
      ...
      <label className="mt-3 flex items-center gap-2 text-sm font-medium text-red-800"><input type="checkbox" name="refund" value="full" defaultChecked className="accent-red-600" /> Refund successful payments</label>
